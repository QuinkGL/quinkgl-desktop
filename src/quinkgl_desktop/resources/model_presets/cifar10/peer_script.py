...
def _ensure_data(data_root: Path, peer_index: int, peers: int = 1) -> None:
    ...
    try:
        full_train = datasets.CIFAR10(raw_root, train=True, download=True, transform=transform)
        full_val = datasets.CIFAR10(raw_root, train=False, download=True, transform=transform)
        ...
        print(f"CIFAR-10 download failed ({exc}); generating local fallback tensors.")
        train_ds = _fallback_tensor_pair(512, (3, 32, 32), 10, seed=1300 + peer_index)
        val_ds = _fallback_tensor_pair(128, (3, 32, 32), 10, seed=1700 + peer_index)
...
def build_optimizer(manifest, model):
    ...
    return torch.optim.SGD(inner.parameters(), lr=0.01, momentum=0.9)
